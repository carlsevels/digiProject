...
SELECT 
    f.id,
    f."folioId",
    substr(datetime(f.created_at, '-6 hours'), 1, 10) AS fecha,
    f.created_at,
    f.cantidad,
...
LEFT JOIN status st
    ON h."statusId" = st.id
WHERE
    substr(datetime(f.created_at, '-6 hours'), 1, 10) = ?
    AND (
        ? = 1
        OR (
            ? = 2
            AND (st.id IS NULL OR st.id != 3)
        )
    );
''';
}
